...
public:
    ...
    static bool valid(const OTAState &s) {
        if (s.phase==OTAPhase::Idle)
            return s.generation==0 && s.sequence==0 && s.releaseId.empty() &&
                s.candidateHash.empty() && s.previousHash.empty() && s.version.empty() && s.failure.empty();
        return s.generation>0 && static_cast<unsigned>(s.phase)<=static_cast<unsigned>(OTAPhase::RolledBack) &&
    ...
    template<class Persist>
    bool accept(const OTAManifest &m, uint32_t generation, const std::string &runningHash, Persist persist) {
        std::string bytes;
        if(!ready_ || !canonicalOTA(m,bytes) || !digest(runningHash) || generation<=state_.generation ||
    ...
    template<class Persist>
    bool booted(const std::string &runningHash, Persist persist) {
        if(!ready_ || !digest(runningHash)) return false;
        ...
        if(state_.phase==OTAPhase::PendingBoot || state_.phase==OTAPhase::Validating) {
            if(runningHash==state_.candidateHash) return change(OTAPhase::Validating,"none",persist);
            if(runningHash==state_.previousHash) return change(OTAPhase::RolledBack,"previous_image_running",persist);
        ...
        if(state_.phase==OTAPhase::Confirmed && runningHash!=state_.candidateHash) {
