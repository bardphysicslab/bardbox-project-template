...
int main() {
    ...
    assert(!machine.accept(m,1,oldHash,2000,save)); // Missing/unprovisioned store.
    assert(machine.restore(durable)); // Explicit commissioning supplies idle.
    assert(!machine.accept(m,1,oldHash,0,save));
    assert(!machine.accept(m,1,oldHash,2097153,save));
    assert(!machine.accept(m,1,m.sha256,m.size+1,save));
    assert(!machine.accept(m,0,oldHash,2000,save));
    assert(machine.accept(m,1,oldHash,2000,save));
    ...
    assert(!machine.accept(m,1,oldHash,2000,save));
    assert(!machine.accept(m,2,oldHash,2000,save)); // Cannot replace an active install.
    ...
    assert(machine.booted(m.sha256,m.size,save));
    ...
    assert(!machine.accept(m,1,oldHash,2000,save));
    assert(machine.accept(m,2,m.sha256,m.size,save)); // Same bytes, no download/reboot.
    ...
    for(bool commitBeforeFailure:{false,true}) {
        ...
        assert(!machine.accept(m,1,oldHash,2000,uncertain));
        assert(!machine.ready());
        assert(!machine.accept(m,1,oldHash,2000,save)); // Cannot act after uncertain write.
        assert(machine.restore(durable));
        assert(machine.booted(oldHash,2000,save));
        if(commitBeforeFailure) {
            assert(durable.phase==OTAPhase::Failed);
            assert(!machine.accept(m,1,oldHash,2000,save));
            assert(machine.accept(m,2,oldHash,2000,save));
        } else assert(machine.accept(m,1,oldHash,2000,save));
    }
    ...
    assert(machine.accept(m,3,oldHash,2000,save));
    assert(machine.downloadedAndVerified(save));
    assert(machine.restore(durable));
    assert(machine.booted(oldHash,2000,save));
    ...
    assert(decoded.candidateBytes==durable.candidateBytes && decoded.previousBytes==durable.previousBytes);
    ...
    auto oldRecord=record; oldRecord[2]='1';
    assert(!OTAStateRecord::decode(oldRecord,decoded));
